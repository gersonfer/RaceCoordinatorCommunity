...
-------	----------  -----------------------------------------------------------
1.0.0 	10/03/2024  Based on RC sketch V1.0.0.13, released w/ Race Coordinator
										version 1.15.2.0, dated 10/10/2023.
										Adds support for Arduino UNO R4 WiFi and ESP32 Development
										Kit/module.  ESP32 code obtained from SlotForum member
										Slingshotx.  UNO R4 WiFi WILL ONLY WORK WITH RC VERSION
										1.15.3.0 AND ABOVE.
*******************************************************************************
*/
...
// Use Bluetooth for a standard ESP32 (not s2 or c3 etc); it needs to support 
// Bluetooth 4 (i.e., Classic Bluetooth, not BLE)
//#define ESP32_BT

/* NOTE: The Arduino development environment has some built-in #defines for
         processor boards.  This means you don't have to list them in your
         sketch; they will be automatically defined based on the board you
         choose to verify/compile within the Arduino IDE.
         In addition tothe "base" Arduino UNO and Mega/Mega 2560, this
         sketch supports:
         
         ESP32									ESP32 Dev Kit and others with Bluetooth 4
         ARDUINO_UNOR4_WIFI
*/
...
/*
For UNO R4 WiFi's 12x8 LED matrix. Installing the 'ArduinoGraphics' library makes
it easier to display alphanumeric characters.
NOTE: #include statement for 'Arduino_LED_Matrix.h' must occur before
			'<FastLED.h>'; otherwise, there will be verify/compile errors.
*/
...
const long wdtInterval = 15;  //set UNO R4 watchdog interval to 15 msec
...
//  fuelRelayPin[7] = 9;  // Lane 8
  ...
  // Add code to display 'RC' on the R4 WiFi's 12x8 LED matrix
  #ifdef ARDUINO_UNOR4_WIFI
  	matrix.begin();
  	matrix.beginDraw();
...
void loop()
{
  ...
  // Mega's and UNO's prior to R4 automatcially call 'serialEvent()'
  // at the end of the main loop.  For others the function needs
  // to be explicitly called.
#if defined ESP_32 || defined ARDUINO_UNOR4_WIFI
	serialEvent();
#endif
  ...
  while (!bRead && Serial.available() > 0)
  {
